...
APP_SALT = "a_super_secret_salt_for_your_app_v3"
...
# --- DATABASE SETUP ---
conn = st.connection("chat_db", type="sql", url="sqlite:///advanced_chat_v3.db", ttl=0)
    ...
    with st.form("change_password_form"):
        ...
        if submitted:
            ...
            if not verify_password(user.hashed_password, current_password):
                ...
                with conn.session as s:
                    s.execute(text("UPDATE users SET hashed_password = :hp WHERE username = :u"), params=dict(hp=new_hashed_password, u=st.session_state.username))
                    s.commit()
...
def show_chat_screen():
    ...
    with tab2:
        ...
        if not locations_df.empty:
            ...
            st.info("No user locations have been shared yet. Click 'Update My Location' in the sidebar to appear on the map!")
